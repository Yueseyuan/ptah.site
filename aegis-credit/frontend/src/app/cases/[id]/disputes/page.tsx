...
import { listDisputeRounds, autoGenerateDisputes, createDisputeRound, updateDisputeRound, createDisputeItem, updateDisputeItem, downloadDisputeLetter, downloadAffidavit, downloadAuthorization } from '@/lib/api';
...
const RECIPIENT_TYPES = [
  { value: 'bureau', label: 'Credit Bureau (Formal Dispute)' },
  { value: 'creditor', label: 'Original Creditor' },
  { value: 'collection_agency', label: 'Collection Agency (Items 3/4/5)' },
  { value: 'debt_buyer', label: 'Debt Buyer (Items 3/4/5)' },
  { value: 'debt_collector', label: 'Debt Collector (Items 3/4/5)' },
  { value: 'failure_to_investigate', label: 'Failure to Investigate (Round 2)' },
  { value: 'method_of_verification', label: 'Method of Verification Demand' },
  { value: 'full_file_disclosure', label: 'Full File Disclosure (§§609-610)' },
  { value: 'cease_desist', label: 'Cease & Desist (§1692c(c))' },
  { value: 'personal_info_dispute', label: 'Personal Info Correction (Standalone)' },
  { value: 'student_loan', label: 'Student Loan Validation (DOE/DMCS)' },
  { value: 'cfpb_complaint', label: 'CFPB Complaint (Full Narrative)' },
  { value: 'cfpb', label: 'CFPB (Short Escalation)' },
  ...
  { value: 'student_loan_servicer', label: 'Student Loan Servicer (HEA)' },
...
export default function DisputesPage() {
  ...
  async function downloadAffidavitDoc() {
    try {
      const blob = await downloadAffidavit(caseId);
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = `affidavit_case${caseId}.txt`;
      a.click();
      URL.revokeObjectURL(url);
    } catch (err: unknown) {
      const e = err as { message?: string };
      setAutoError(e.message || 'Affidavit download failed.');
    }
  }

  async function downloadAuthorizationDoc() {
    try {
      const blob = await downloadAuthorization(caseId);
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = `authorization_case${caseId}.txt`;
      a.click();
      URL.revokeObjectURL(url);
    } catch (err: unknown) {
      const e = err as { message?: string };
      setAutoError(e.message || 'Authorization download failed.');
    }
  }
      ...
      <main className="main-content">
        <div className="page-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            <button className="btn btn-outline" onClick={downloadAffidavitDoc} title="Download sworn affidavit (28 U.S.C. §1746)">📄 Affidavit</button>
            <button className="btn btn-outline" onClick={downloadAuthorizationDoc} title="Download authorization / limited POA letter">📋 Authorization</button>
